...
def process_pair(path1, path2, pair_number):
    ...
    if des1 is None or des2 is None:
        ...
        print("No descriptors found in one of the images - cannot compare.")
    ...
    if total_features > 0:
        ...
        similarity = (len(good_matches) / total_features) * 100
    ...
    print(f"Total keypoints considered: {total_features}")

    print(f"Good feature matches: {len(good_matches)}")

    print(f"Feature similarity: {similarity:.2f}%")
...
def find_pairs():
    ...
    for base_name in sorted(groups):
        ...
        if "1" in group and "2" in group:
            ...
            image1 = os.path.join(INPUT_DIR, group["1"])

            image2 = os.path.join(INPUT_DIR, group["2"])

            pairs.append((image1, image2))

            print(f"Found pair: "f"{group['1']} <-> {group['2']}")

        else:

            print(f"Incomplete pair: {base_name}")
...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        process_pair( sys.argv[1], sys.argv[2], 1)
    ...
    for i, (path1, path2) in enumerate(pairs, start=1):

        process_pair(path1, path2, i)

    print("\n======================================")

    print(f"Finished matching {len(pairs)} pairs.")

    print(f"Match images saved in {OUTPUT_DIR}")

    print(f"Comparison results saved in {COMPARISON_DIR}")
